...
def generate_launch_description():
    ...
    declare_use_joint_state_gui_cmd = DeclareLaunchArgument(
        'use_joint_state_gui',
        default_value='False',
        description='whether to use the joint-state-publisher-gui package. only use with rviz only'
    )
    ...
    rviz_launch = IncludeLaunchDescription(
                PythonLaunchDescriptionSource(
                    [os.path.join(my_pkg_path,'launch','rviz.launch.py')]
                    ), 
                launch_arguments={'use_sim_time': use_sim_time,
                                  'use_rviz_only': use_rviz_only,
